...
class SASRecD(torch.nn.Module):
    ...
    def forward(self, item_seq, item_seq_len, cate_seq):
        ...
        output = trm_output[-1]
        # seq_output = self.gather_indexes(output, item_seq_len)
        #[]
        return output
    ...
    def calculate_loss(self, item_seq, item_seq_len, cate_seq, pos_items, pos_cate):
        ...
        valid_numpy = np.array(item_seq.to('cpu'))>0
        label_numpy = valid_numpy.nonzero()
        seq_output = seq_output[label_numpy]
        #[m,d]
        # pos_items = interaction[self.POS_ITEM_ID]
        if True:  # self.loss_type = 'CE'
            test_item_emb = self.item_embedding.weight
            logits = torch.matmul(seq_output, test_item_emb.transpose(0, 1))
            #pos_items should be [b,h]
            loss = self.loss_fct(logits, pos_items[label_numpy])
            ...
            if self.attribute_predictor!='' and self.attribute_predictor!='not':
                ...
                for i, a_predictor in enumerate(self.ap):
                    ...
                    #pos_cate should be [b,h]
                    attribute_labels = pos_cate[label_numpy]
                    #[m]
    ...
    def full_sort_predict(self, item_seq, item_seq_len, cate_seq):
        ...
        # item_seq_len = interaction[self.ITEM_SEQ_LEN]
        seq_output = self.forward(item_seq, item_seq_len, cate_seq)
        seq_output = self.gather_indexes(seq_output, item_seq_len)
...
class casr(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self.itemmodel.calculate_loss(item_seq_batch, item_seq_len, cate_seq_batch, itemy_seq_batch, catey_seq_batch)
...
trainer = pl.Trainer(devices=[args.cuda], callbacks=[early_stop_callback, modelcheckpoint], accelerator="gpu", accumulate_grad_batches=1, max_epochs=200,  default_root_dir='./final/{}/difsr_ce/recall{}/lr{}'.format(args.datasets , args.recall, args.lr))
