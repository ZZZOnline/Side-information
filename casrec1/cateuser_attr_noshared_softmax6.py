...
class cudaSASRec(torch.nn.Module):
    def __init__(self, user_num, item_num, args):
        ...
        self.feed_forward = PointWiseFeedForward(args.hidden_size, args.dropout_rate)
...
class PASR(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # user_cate = self.catemodel.feature_gating(self.itemmodel.cate_emb(cate_seq_batch), self.catemodel.user_emb(user_batch))[:,-1,:].unsqueeze(1)

        #behasum1
        cate_fuse = self.catemodel.feature_fuse(self.itemmodel.cate_emb(cate_y_batch.view(-1,1)), cate_logits[:, -1, :].unsqueeze(1))

        # print(cate_fuse.shape)
        
        iteminput = self.itemmodel.cateitem_emb(item_seq_batch) + self.itemmodel.catecate_emb(cate_seq_batch)

        cate_input = self.starmodel(iteminput, self.itemmodel.cate_emb(cate_seq_batch), cate_fuse , mask)
        ...
        catetest_item_emb = self.itemmodel.cateitem_emb.weight

        catetest_cate_emb = self.itemmodel.catecate_emb(self.itemmodel.cateall.to(self.args.device))

        logits = torch.einsum('cd,bd -> bc', catetest_item_emb+catetest_cate_emb , starinput)

        itemloss = self.loss(logits, item_y_batch)


        # print(torch.mean(floss), torch.mean(itemloss))
        loss = args.gamma*floss + (1-args.gamma)*itemloss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        pred_cate_emb = self.itemmodel.cate_emb(pred_cate_index)
        ...
        catetest_item_emb = self.itemmodel.cateitem_emb.weight
        catetest_cate_emb = self.itemmodel.catecate_emb(self.itemmodel.cateall.to(self.args.device))
        catelogits = torch.einsum('cd,bhd -> bhc', catetest_item_emb +catetest_cate_emb, cate_input)
    ...
    def test_step(self, batch, batch_idx):
        ...
        # print(next(self.featmodel[num].parameters()).device)
        cate_logits = self.catemodel.log2feats(cate_seq_batch)
        user_emb = self.itemmodel.user_emb(user_batch.view(-1,1))
        user_cate = self.catemodel.feature_gating(self.itemmodel.cate_emb(cate_seq_batch), self.catemodel.user_emb(user_batch))[:,-1,:].unsqueeze(1)
...
if args.datasets=='taobao':
    trainer = pl.Trainer(devices=[args.cuda], callbacks=[modelcheckpoint], accelerator="gpu", accumulate_grad_batches=1, max_epochs=200,  default_root_dir='./final/{}/cu_noshared6/beam{}/recall{}/lr{}'.format(args.datasets ,args.beam_size, args.recall, args.lr))
else:
    trainer = pl.Trainer(devices=[args.cuda], callbacks=[early_stop_callback, modelcheckpoint], accelerator="gpu", accumulate_grad_batches=1, max_epochs=200,  default_root_dir='./final/{}/cu_noshared6/gamma{}/beam{}/recall{}/lr{}'.format(args.datasets, args.gamma ,args.beam_size, args.recall, args.lr))
