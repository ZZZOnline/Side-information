...
class PASR(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        cate_input = self.starmodel(self.itemmodel.cateitem_emb(item_seq_batch), self.catemodel.item_emb(cate_seq_batch), cate_logits[:, -1, :].unsqueeze(1) , mask)
        ...
        logits = torch.einsum('cd,bd -> bc', catetest_item_emb , starinput)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        pred = torch.einsum('cd,bd -> bc', catetest_item_emb , cate_input.squeeze())



        valid_labels = torch.ones((pred.shape[0])).long().to(args.device)

        # metrics = evaluate(pred, item_y_batch, torch.tensor([True]).expand_as(valid_labels).to('cuda'), recalls=args.recall)
        metrics = evaluate(pred, item_y_batch, torch.tensor([True]).expand_as(valid_labels).to(self.args.device), recalls=args.recall)
...
if args.datasets=='taobao':
    trainer = pl.Trainer(devices=[args.cuda], callbacks=[modelcheckpoint], accelerator="gpu", accumulate_grad_batches=1, max_epochs=200,  default_root_dir='./final/{}/cu_shared4/beam{}/recall{}/lr{}'.format(args.datasets ,args.beam_size, args.recall, args.lr))
else:
    trainer = pl.Trainer(devices=[args.cuda], callbacks=[early_stop_callback, modelcheckpoint], accelerator="gpu", accumulate_grad_batches=1, max_epochs=200,  default_root_dir='./final/{}/cu_shared4/beam{}/recall{}/lr{}'.format(args.datasets ,args.beam_size, args.recall, args.lr))
