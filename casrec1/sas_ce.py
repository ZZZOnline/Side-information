...
class casr(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        item_logits = self.itemmodel.log2feats(item_seq, mask)

        outputs = self.itemmodel.heads(item_logits, None)

        loss = self.loss(outputs[label_numpy], itemy_seq_batch[label_numpy])
    ...
    def validation_step(self, batch, batch_idx):
        ...
        logits = self.itemmodel.log2feats(item_seq, mask)


        logit = self.itemmodel.head(logits[:,-1,:], None)
        # pred_item_prob1 = self.m_logsoftmax(outputs)
        # print('1',logit.shape)
        # pred_item_prob1 = self.m_logsoftmax(logit)
        # print('2',pred_item_prob1.shape)
...
trainer = pl.Trainer(devices=[args.cuda], callbacks=[early_stop_callback, modelcheckpoint], accelerator="gpu", accumulate_grad_batches=1, max_epochs=200,  default_root_dir='./final/{}/sas_ce/recall{}/lr{}'.format(args.datasets , args.recall, args.lr))
