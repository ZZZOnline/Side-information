...
class casr(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        usermask = torch.bernoulli(torch.full(user_batch.shape, 0.92)).to(self.args.device)
        ...
        loss = self.loss(scores, item_y_batch)
...
trainer = pl.Trainer(devices=[args.cuda], callbacks=[early_stop_callback, modelcheckpoint], accelerator="gpu", accumulate_grad_batches=1, max_epochs=200,  default_root_dir='./final/{}/ssept/userprob{}/lr{}'.format(args.datasets , args.user_prob, args.lr))
