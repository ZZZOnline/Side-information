...
class cudaSASRec(torch.nn.Module):
    def __init__(self, user_num, item_num, args):
        ...
        self.newattention_layernorms = torch.nn.ModuleList() # to be Q for self-attention
        self.newattention_layers = torch.nn.ModuleList()
        self.newforward_layernorms = torch.nn.ModuleList()
        self.newforward_layers = torch.nn.ModuleList()
        self.newfeed_forward = PointWiseFeedForward(args.hidden_units, args.dropout_rate)

        self.newlast_layernorm = torch.nn.LayerNorm(args.hidden_units, eps=1e-8)
        self.newdropout = torch.nn.Dropout(p = args.dropout_rate)

        for _ in range(args.num_blocks):
            new_attn_layernorm = torch.nn.LayerNorm(args.hidden_units, eps=1e-8)
            self.newattention_layernorms.append(new_attn_layernorm)

            new_attn_layer =  torch.nn.MultiheadAttention(args.hidden_units,
                                                            args.num_heads,
                                                            args.dropout_rate, batch_first=True)
            self.newattention_layers.append(new_attn_layer)

            new_fwd_layernorm = torch.nn.LayerNorm(args.hidden_units, eps=1e-8)
            self.newforward_layernorms.append(new_fwd_layernorm)

            new_fwd_layer = PointWiseFeedForward(args.hidden_units, args.dropout_rate)
            self.newforward_layers.append(new_fwd_layer)
    ...
    def log2feats(self, log_seqs, log_value, lastvalue_seqs):

        seqs = self.item_emb(log_seqs)
        value_seqs = self.item_emb(log_value)
        ...
        value_seqs *= ~timeline_mask.unsqueeze(-1)
        ...
        for i in range(len(self.attention_layers)):
            ...
            mha_outputs, _ = self.attention_layers[i](seqs, seqs, value_seqs, 
...
class PASR(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        cate_logits = self.itemmodel.log2feats(item_seq_batch, item_seq_batch, item_seq_batch)
        cate_input = cate_logits
        cate_outputs = self.itemmodel.heads(cate_input, None)     

        loss = self.loss(cate_outputs[:,-1,:], item_y_batch)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        cate_logits = self.itemmodel.log2feats(item_seq_batch, item_seq_batch, item_seq_batch)[:,-1,:].unsqueeze(1)
        # print(cate_logits.shape)
        cate_input = cate_logits
        outputs = self.itemmodel.heads(cate_input, None)     

        valid_labels = torch.ones((outputs.shape[0])).long().to(args.device)

        # print(outputs.shape)
        # print(item_y_batch.shape)
        # print(valid_labels.shape)
        metrics = evaluate(outputs.squeeze(), item_y_batch, torch.tensor([True]).expand_as(valid_labels).to(self.args.device), recalls=args.recall)

        # cate_recall, cate_mrr, cate_ndcg = evaluate_part(outputs, item_y_batch, torch.tensor([True]).expand_as(valid_labels).to('cuda'), k=10)
...
recmodel = PASR(args, usernum, itemnum, itemnum)
...
trainer = pl.Trainer(devices=[args.cuda], callbacks=[early_stop_callback, modelcheckpoint], accelerator="gpu", accumulate_grad_batches=1, max_epochs=100,  default_root_dir='./final/{}/iu_pre1/{}/beam{}/gamma{}/recall{}/lr{}'.format(args.datasets, args.preference, args.beam_size, args.gamma, args.recall, args.lr))
