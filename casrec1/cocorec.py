...
class casr(pl.LightningModule):
    ...
    def training_epoch_end(self, training_step_outputs):
        ...
        # if self.train_epoch - self.max_cate_epoch > 5:
    ...
    def validation_step(self, batch, batch_idx):
        if not self.m_teacher_forcing_flag:
            args = self.args
            user_seq_batch, item_y_batch, cate_y_batch,  item_seq_batch, cate_seq_batch,  pad_incateitem_seq_batch, pad_cateset_seq_batch, itemy_seq_batch, catey_seq_batch = batch
    ...
    def validation_epoch_end(self, validation_step_outputs):
        if not self.m_teacher_forcing_flag:
            ...
            qwe = validation_step_outputs[0]['Val:Recall@10']
            # self.log('validcate_recall', validcate_recall, on_epoch=True)
            self.log(f'Val:Recall@10', torch.Tensor(qwe).mean() , on_epoch=True)
...
trainer = pl.Trainer(devices=[args.cuda], callbacks=[modelcheckpoint], accelerator="gpu", accumulate_grad_batches=1, max_epochs=200,  default_root_dir='./final/{}/cocorec/recall{}/lr{}'.format(args.datasets , args.recall, args.lr))
