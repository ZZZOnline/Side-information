...
parser.add_argument('--integra', type=str, default='item')
...
class cudaSASRec(torch.nn.Module):
    def __init__(self, user_num, item_num, args):
        ...
        self.w5 = nn.Linear(2*args.hidden_units, args.hidden_units)
    ...
    def origingating(self, item_logits, fea_logits):
...
class starcateseq(torch.nn.Module):
    ...
    def forward(self, item_seq, cate_seq, cate_y, mask):
        ...
        att_out = self.dropout(torch.cat(torch.split(att_out, item_seq.shape[0], dim=0), dim=2)) # div batch_size
...
class PASR(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if args.integra =='origingating':
            iteminput = self.itemmodel.origingating(self.itemmodel.cateitem_emb(item_seq_batch), self.catemodel.item_emb(cate_seq_batch))

        if args.integra =='add':
            iteminput = self.itemmodel.cateitem_emb(item_seq_batch)+ self.catemodel.item_emb(cate_seq_batch)

        if args.integra =='concat':
            iteminput = self.itemmodel.concat(self.itemmodel.cateitem_emb(item_seq_batch), self.catemodel.item_emb(cate_seq_batch))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        catelogits = torch.einsum('cd, bhd -> bhc', allinput  , cate_input)
...
early_stop_callback = EarlyStopping(monitor="Val:Recall@10", min_delta=0.00, patience=50, verbose=False, mode="max")
...
# else:
trainer = pl.Trainer(devices=[args.cuda], callbacks=[early_stop_callback, modelcheckpoint], accelerator="gpu", accumulate_grad_batches=1, max_epochs=100,  default_root_dir='./ffinal/{}/newcasrec_drop/{}/beam{}/gamma{}/recall{}/lr{}'.format(args.datasets, args.integra, args.beam_size, args.gamma, args.recall, args.lr))
