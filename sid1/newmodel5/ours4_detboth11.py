...
#1 means different query for cp and pc
#11 means no dropout

class ours4_detboth11(torch.nn.Module):
    r"""
    ...
    def __init__(self, config, dataset, args):
        super(ours4_detboth11, self).__init__()
    ...
    def decoder(self, input, value_input, beha_emb, extended_attention_mask):
        ...
        c2b_att = torch.matmul(query_layer, beha_key_layer.transpose(-1, -2))
        attention_scores += (c2p_att + c2b_att)/2


        #c2p
        p2c_att = torch.matmul(pos_query_layer, key_layer.transpose(-1, -2))
        p2c_att = torch.gather(p2c_att, dim=-2, index=c2p_pos.transpose(-1, -2))
    ...
    def calculate_loss(self, interaction):
        ...
        # itemoutput squeezed
        itemoutput, fea_output, origin_itemoutput = self.forward(item_seq, item_seq_len, beha_seq1)
        args = self.args
