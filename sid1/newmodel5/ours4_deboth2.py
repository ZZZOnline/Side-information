...
class ours4_deboth2(torch.nn.Module):
    r"""
    ...
    def __init__(self, config, dataset, args):
        super(ours4_deboth2, self).__init__()
        ...
        self.relative_pos = build_relative_position(self.max_seq_length, self.max_seq_length, -1)
        ...
        self.beha_embedding = nn.Embedding((self.n_beha-1)**2+1, self.hidden_size, padding_idx=0)
        self.item_embedding = nn.Embedding(self.n_items, self.hidden_size, padding_idx=0)
        self.position_embedding = nn.Embedding(self.max_seq_length, self.hidden_size)
        self.position_embedding1 = nn.Embedding(self.max_seq_length*2, self.hidden_size)
    ...
    def decoder(self, input, value_input, beha_emb, beha_emb2, extended_attention_mask):
        
        rel_embeddings = self.position_embedding1.weight[0:2*self.max_seq_length, :].unsqueeze(0).repeat(input.size(0),1,1)

        pos_emb = self.dropout(rel_embeddings)
        ...
        # print(pos_query_layer.shape, pos_key_layer.shape)
        #bhnd
        ...
        beha_key_layer2 = self.transpose_for_scores(self.beha_key_proj(beha_emb2))
        # beha_query_layer2 = self.transpose_for_scores(self.beha_query_proj(beha_emb2))
        # Take the dot product between "query" and "key" to get the raw attention scores.
        c2p_pos = torch.clamp(self.relative_pos.to(query_layer.device) + self.max_seq_length, 0, self.max_seq_length*2-1).unsqueeze(0).expand([query_layer.size(0), query_layer.size(1), self.relative_pos.size(-2), self.relative_pos.size(-1)])
        # print(c2p_pos.shape, c2p_pos.device)
       
        #[b,n,d] [b,d,n], [b,1,d] [0,1],  [98*0] [97*1]
        ...
        #[b,h,n,d] [b,h,n,d]
        c2b_att = torch.matmul(query_layer, beha_key_layer2.transpose(-1, -2))
        attention_scores += (c2p_att + c2b_att)


        #c2p
        p2c_att = torch.matmul(pos_query_layer, key_layer.transpose(-1, -2))
        p2c_att = torch.gather(p2c_att, dim=-2, index=c2p_pos)
        b2c_att = torch.einsum('bhnd, bhnd->bhn',beha_query_layer, key_layer)
        # torch.matmul(beha_query_layer, beha_key_layer.transpose(-1, -2))

        # print(p2c_att.shape, b2c_att.shape)
        attention_scores += (p2c_att+b2c_att.unsqueeze(-2))
    
        attention_scores = attention_scores / math.sqrt(self.attention_head_size*5)
    ...
    def decoder1(self, input, value_input, beha_emb, beha_emb2, extended_attention_mask):
        rel_embeddings = self.position_embedding1.weight[0:2*self.max_seq_length, :].unsqueeze(0).repeat(input.size(0),1,1)
        ...
        # Take the dot product between "query" and "key" to get the raw attention scores.
        # attention_scores = torch.matmul(query_layer[:,:,-1,:].unsqueeze(-2), key_layer.transpose(-1, -2))
        # +\
        #             torch.einsum('bhijd, bhjd->bhi', pos_query_layer, key_layer).unsqueeze(1)
        # +\
        #             torch.einsum('bhid, bhijd->bhi', query_layer, pos_key_layer).unsqueeze(1)
    ...
    def forward(self, item_seq, item_seq_len,  beha_seq, beha_seq2):
        args = self.args
        ...
        beha_emb = self.dropout(self.beha_embedding(beha_seq))
        beha_emb2 = self.dropout(self.beha_embedding(beha_seq2))
        # beha_emb = None
        hidden_states = self.decoder(output, item_emb, beha_emb, beha_emb2, extended_attention_mask)
        ...
        fea_output = []
        for i, name in enumerate(self.selected_features):
            fea_hidden = self.decoder1(output, feature_table[i].squeeze(), beha_emb, beha_emb2, extended_attention_mask)
    ...
    def calculate_loss(self, interaction):
        ...
        beha_seq1= self.delta_seq(beha_seq, item_seq_len)
        # itemoutput squeezed
        beha_seq2 = self.delta_seq(beha_seq, item_seq_len)
        itemoutput, fea_output, origin_itemoutput = self.forward(item_seq, item_seq_len, beha_seq1, beha_seq2)
        args = self.args
    ...
    def delta_seq(self, b_seq, seq_len):
        b_seq1 = b_seq.gather(dim=1, index=seq_len[:,None]-1)
        b_seq2 = (b_seq + (b_seq1-1)*(self.n_beha-1)) * (b_seq!=0)
        return b_seq2
    
    def delta_seq2(self, b_seq, seq_len):
        b_seq1 = b_seq.gather(dim=1, index=seq_len[:,None]-1)
        b_seq2 = (b_seq1 + (b_seq-1)*(self.n_beha-1)) * (b_seq!=0)
        return b_seq2