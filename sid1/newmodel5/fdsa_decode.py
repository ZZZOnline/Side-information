...
class fdsa_decode(torch.nn.Module):
    """
    ...
    def __init__(self, config, dataset, args):
        super(fdsa_decode, self).__init__()
        ...
        self.feat_emb_layers1 = nn.ModuleList()
        self.feat_all = []
        self.feanum = []
        ...
        for i, field_name in enumerate(config['selected_features']):
            ...
            if self.dataset.field2type[field_name] == FeatureType.TOKEN:
                ...
                # self.feat_emb_layers1.append(nn.Embedding(self.dataset.num(field_name), self.attribute_hidden_size[i], padding_idx=0))
    ...
    def forward(self, item_seq, item_seq_len, beha_seq):
        ...
        fea_output = []
        for i, name in enumerate(self.selected_features):
            fea_hidden = self.decoder1(output, feature_table1[i].squeeze(), beha_emb, extended_attention_mask)
            fea_hidden = self.gather_indexes(fea_hidden, item_seq_len - 1).unsqueeze(1)
            fea_hidden = self.transpose_for_heads(fea_hidden, self.args.p_heads)
        ...
        return itemoutput.squeeze(), fea_output, output  # [B H]
