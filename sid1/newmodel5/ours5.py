...
class ours5(torch.nn.Module):
    ...
    def __init__(self, config, dataset, args):
        ...
        self.gating_network1 = VanillaAttention2(self.hidden_size, self.hidden_size)
    ...
    def gating1(self, item_logits, fea_logits, boost):
        ...
        weigh =self.gating_network1(input)
    ...
    def decoder(self, input, value_input, beha_emb, extended_attention_mask):
        ...
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))+torch.matmul(query_layer0, (beha_key_layer+pos_key_layer).transpose(-1, -2)/2)+\
                    torch.matmul((beha_query_layer+ pos_query_layer)/2, key_layer0.transpose(-1, -2))
    ...
    def decoder1(self, input, value_input, beha_emb, extended_attention_mask):
        ...
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))+torch.matmul(query_layer0, pos_key_layer.transpose(-1, -2))+\
                    torch.matmul( pos_query_layer, key_layer0.transpose(-1, -2))
    ...
    def calculate_loss(self, interaction):
        ...
        if self.args.integra =='gating1':
            boost = torch.cat([p for p in self.feat_boost], dim=-2)
            boost = torch.cat((self.boost1, boost), dim=-2).squeeze()
            fea_input = torch.cat(fea_output, dim=-2)
            # print(fea_input.shape, item_logits.shape)
            input = torch.cat([itemoutput.unsqueeze(-2), fea_input], dim=-2)
            weights =self.gating_network1(input, boost)
            fuse_emb = (input * weights.unsqueeze(-1)).sum(dim=-2)
        ...
        if self.args.integra =='gating1':
            boost = torch.cat([p for p in self.feat_boost], dim=-2)
            boost = torch.cat((self.boost1, boost), dim=-2).squeeze()
            fea_embs = torch.cat(feature_emb, dim=-2)
            # print(fea_input.shape, item_logits.shape)
            catetest_item_emb = self.item_embedding.weight.unsqueeze(1)
            input = torch.cat([catetest_item_emb, fea_embs], dim=-2)
            weights =self.gating_network1(input, boost)

            allinput = (input * weights.unsqueeze(-1)).sum(dim=-2)
