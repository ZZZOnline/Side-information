...
class bert4rec_decode(torch.nn.Module):
    ...
    def __init__(self, config, dataset, args):
        ...
        for i, field_name in enumerate(config['selected_features']):
            ...
            if self.dataset.field2type[field_name] == FeatureType.TOKEN:
                ...
                self.feat_emb_layers.append(nn.Embedding(self.dataset.num(field_name)+1, self.attribute_hidden_size[i], padding_idx=0))
        ...
        self.feat_all0 = []
        print(self.feat_all)
        for i in self.feat_all:
            print(len(i.size()))
            if len(i.size())==2:
                a = torch.zeros( (1, i.shape[1]), dtype =i.dtype)
                self.feat_all0.append(torch.cat((i,a),dim=0))
            else:
                a = torch.zeros( (1), dtype =i.dtype)
                self.feat_all0.append(torch.cat((i,a),dim=0))  
    ...
    def reconstruct_test_data(self, item_seq, item_seq_len):
        ...
        return item_seq
    def decoder(self, input, value_input,  extended_attention_mask):
    ...
    def decoder1(self, input, value_input,  extended_attention_mask):
        position_ids = torch.arange(input.size(1), dtype=torch.long, device=input.device)
        position_ids = position_ids.unsqueeze(0).expand(input.size()[:-1])
        position_embedding = self.position_embedding1(position_ids)
        pos_emb = position_embedding
    ...
    def forward(self, item_seq):
        ...
        for i, name in enumerate(self.selected_features):
            if self.dataset.field2type[name] == FeatureType.TOKEN:
                feature_table.append(self.feat_emb_layers[i](self.feat_all0[i][item_seq].to(item_seq.device)).unsqueeze(-2))

            elif self.dataset.field2type[name] == FeatureType.TOKEN_SEQ:
                # self.token_seq_field_names[type].append(field_name)
                if self.args.pooling_mode=='sum':
                    feat_emb = torch.sum(self.feat_emb_layers[i](self.feat_all0[i][item_seq].to(item_seq.device)), dim=-2).unsqueeze(-2)
                ...
                #0 padding should not be considered in mean function
                    ...
                    token_seq_field = self.feat_all0[i][item_seq].to(item_seq.device)
        ...
        hidden_states = self.decoder(output, item_emb, extended_attention_mask)
        itemoutput = hidden_states + output
        itemoutput = self.transpose_for_heads(itemoutput, self.args.p_heads)
        ...
        fea_output = []
        for i, name in enumerate(self.selected_features):
            fea_hidden = self.decoder1(output, feature_table[i].squeeze(),  extended_attention_mask)
        ...
        return hidden_states, fea_output  # [B L H]
    ...
    def calculate_loss(self, interaction):
        ...
        seq_output, fea_output = self.forward(masked_item_seq)
        ...
        itemoutput = torch.bmm(pred_index_map, seq_output)  # [B mask_len H]
        # print(itemoutput.shape)
        fea_outputs = []
        for i in fea_output:
            fea_outputs.append(torch.bmm(pred_index_map, i).unsqueeze(-2))
            # print(torch.bmm(pred_index_map, i).shape)
        # seq_output = torch.bmm(pred_index_map, fea_output)

        if self.args.integra =='add':
            fuse_emb = self.sum(itemoutput, fea_outputs) 
        ...
        catetest_item_emb = self.item_embedding.weight[:self.n_items].unsqueeze(1)  # [item_num H]
        ...
        loss = torch.sum(loss_fct(logits.view(-1, allinput.size(0)), pos_items.view(-1)) * targets) \
                / torch.sum(targets)
        ...
        for i in self.feat_boost:
            boostloss += i.norm()
        return loss+ self.weight_norm*(boostloss)
    ...
    def full_sort_predict(self, interaction):
        ...
        item_seq_len = interaction[self.ITEM_SEQ_LEN]
        item_seq = self.reconstruct_test_data(item_seq, item_seq_len)
        seq_output, fea_output = self.forward(item_seq)
        ...
        fea_outputs = []
        for i in fea_output:
            fea_outputs.append(self.gather_indexes(i, item_seq_len - 1))
            # print(torch.bmm(pred_index_map, i).shape)
        # seq_output = torch.bmm(pred_index_map, fea_output)

        if self.args.integra =='add':
            fuse_emb = self.sum(seq_output, fea_outputs) 
