...
class sasrec_posv(torch.nn.Module):
    r"""
    ...
    def __init__(self, config, dataset, args):
        super(sasrec_posv, self).__init__()
    ...
    def forward(self, item_seq, item_seq_len):
        ...
        fuse_emb = item_emb+position_embedding


        v_emb = self.dropout(fuse_emb)

        q_input = self.dropout(item_emb)

        extended_attention_mask = self.get_attention_mask(item_seq)

        for i in range(1):

            output = self.attention_layers[i](q_input, extended_attention_mask, v_emb)
