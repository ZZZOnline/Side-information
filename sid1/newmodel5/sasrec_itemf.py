...
class sasrec_itemf(torch.nn.Module):
    r"""
    ...
    def __init__(self, config, dataset, args):
        super(sasrec_itemf, self).__init__()
    ...
    def forward(self, item_seq, item_seq_len):
        ...
        # input_emb = self.dropout(position_embedding)
        # input_emb = fuse_emb + position_embedding
        # input_emb = self.LayerNorm(input_emb)
        # input_emb = self.dropout(input_emb)

        item_input = self.dropout(fuse_emb)

        extended_attention_mask = self.get_attention_mask(item_seq)

        for i in range(1):
            item_input = self.attention_layers[i](item_input, extended_attention_mask, item_input)
