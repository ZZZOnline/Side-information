...
class ours1_pre(torch.nn.Module):
    r"""
...
#         tensor([[[2.1613]]], device='cuda:0', requires_grad=True) Parameter containing:
# tensor([[[0.8717]]],
        
        #0.1681 0.0134
        self.boost1 = torch.nn.Parameter(torch.ones((self.p_heads, 1, 1), requires_grad=False)*2.1613)
        self.boost2 = torch.nn.Parameter(torch.ones((self.p_heads, 1, 1), requires_grad=False)*0.8717)
    ...
    def forward(self, item_seq, item_seq_len, beha_seq):
        ...
        # print(itemoutput.shape)
        # print('boost1', self.boost1.shape)
        ...
        for i, name in enumerate(self.selected_features):
            ...
            fea_hidden = torch.einsum('bnhdl, hlk->bnhdk', fea_hidden, self.boost2)
            fea_hidden = fea_hidden.view(*new_shape)

            fea_output.append(fea_hidden)
        
        # itemoutput squeezed
        return itemoutput.squeeze(), fea_output
    ...
    def calculate_loss(self, interaction):
        ...
        for i, name in enumerate(self.selected_features):
            ...
            result = torch.einsum('bnhdl, hlk->bnhdk', result, self.boost2)
