...
class ours4_realmore22both0(torch.nn.Module):
    r"""
    ...
    def __init__(self, config, dataset, args):
        super(ours4_realmore22both0, self).__init__()
    ...
    def decoder2(self, input, value_input, beha_emb, extended_attention_mask):
        position_ids = torch.arange(input.size(1), dtype=torch.long, device=input.device)
        position_ids = position_ids.unsqueeze(0).expand(input.size()[:-1])
        position_embedding = self.position_embedding1(position_ids)
        pos_emb = position_embedding
        ...
        # Take the dot product between "query" and "key" to get the raw attention scores.
        pos_key_layer = self.transpose_for_scores(self.pos_key_proj2(pos_emb))
        pos_query_layer = self.transpose_for_scores(self.pos_query_proj2(pos_emb))

        beha_key_layer = self.transpose_for_scores(self.beha_key_proj2(beha_emb))
        beha_query_layer = self.transpose_for_scores(self.beha_query_proj2(beha_emb))
