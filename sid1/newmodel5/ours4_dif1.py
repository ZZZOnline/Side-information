...
#with beha embedding
class ours4_dif1(torch.nn.Module):
    r"""
    ...
    def __init__(self, config, dataset, args):
        super(ours4_dif1, self).__init__()
    ...
    def decoder(self, input, value_input, beha_emb, extended_attention_mask):
        ...
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))+torch.matmul(pos_query_layer, pos_key_layer.transpose(-1, -2))
