...
class ours4_deboth(torch.nn.Module):
    r"""
    ...
    def __init__(self, config, dataset, args):
        super(ours4_deboth, self).__init__()
    ...
    def decoder(self, input, value_input, beha_emb, beha_emb2, extended_attention_mask):
        ...
        #c2p
        c2p_att = torch.matmul(query_layer0, pos_key_layer.transpose(-1, -2))
        c2p_att = torch.gather(c2p_att, dim=-1, index=c2p_pos)

        #[b,h,n,d] [b,h,n,d]
        c2b_att = torch.matmul(query_layer0, beha_key_layer2.transpose(-1, -2))
        attention_scores += (c2p_att + c2b_att)/2


        #c2p
        p2c_att = torch.matmul(pos_query_layer, key_layer0.transpose(-1, -2))
        p2c_att = torch.gather(p2c_att, dim=-2, index=c2p_pos)
        b2c_att = torch.einsum('bhnd, bhnd->bhn',beha_query_layer, key_layer0)
        # torch.matmul(beha_query_layer, beha_key_layer.transpose(-1, -2))

        # print(p2c_att.shape, b2c_att.shape)
        attention_scores += (p2c_att+b2c_att.unsqueeze(-2))/2
