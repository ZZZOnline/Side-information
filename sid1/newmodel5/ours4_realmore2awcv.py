...
#same as 01,just for clarify
class ours4_realmore2awcv(torch.nn.Module):
    r"""
    ...
    def __init__(self, config, dataset, args):
        super(ours4_realmore2awcv, self).__init__()
        ...
        self.query20 = nn.Linear(self.hidden_size, self.all_head_size)
        self.key20 = nn.Linear(self.hidden_size, self.all_head_size)
        
        self.value2 = nn.Linear(self.hidden_size, self.all_head_size)
        self.pos_key_proj2 = nn.Linear(self.hidden_size, self.all_head_size, bias=True)
        self.pos_query_proj2 = nn.Linear(self.hidden_size, self.all_head_size)
        self.beha_key_proj2 = nn.Linear(self.hidden_size, self.all_head_size)
        self.beha_query_proj2 = nn.Linear(self.hidden_size, self.all_head_size)
    ...
    def decoder2(self, input, value_input, fea_emb, extended_attention_mask):
        ...
        value_input = self.dropout(pos_emb )
        mixed_query_layer = self.query2(input)
        mixed_key_layer = self.key2(input)

        mixed_query_layer0 = self.query20(input)
        mixed_key_layer0 = self.key20(input)

        mixed_value_layer = self.value2(value_input)+value_input
        ...
        beha_key_layer = self.transpose_for_scores(self.beha_key_proj1(fea_emb))
        beha_query_layer = self.transpose_for_scores(self.beha_query_proj1(fea_emb))

        # Take the dot product between "query" and "key" to get the raw attention scores.
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))+torch.matmul(query_layer0, beha_key_layer.transpose(-1, -2))+\
                    torch.matmul(beha_query_layer, key_layer0.transpose(-1, -2))+torch.matmul(beha_query_layer, beha_key_layer.transpose(-1, -2))
    ...
    def decoder(self, input, value_input, fea_emb, extended_attention_mask):
        ...
        beha_key_layer = self.transpose_for_scores(self.beha_key_proj(fea_emb))
        beha_query_layer = self.transpose_for_scores(self.beha_query_proj(fea_emb))
        # Take the dot product between "query" and "key" to get the raw attention scores.
    ...
    def forward(self, item_seq, item_seq_len, beha_seq):
        ...
        beha_emb = None
        hidden_states = self.decoder(output, item_emb, feature_table[0].squeeze(), extended_attention_mask)
        ...
        fea_hidden = self.decoder2(output, None, feature_table[0].squeeze(), extended_attention_mask)
        ...
        fea_hidden = torch.einsum('bnhdl, hlk->bnhdk', fea_hidden, self.feat_boost[1])
    ...
    def calculate_loss(self, interaction):
        ...
        final_output = self.new_fwd_layer(fuse_emb).squeeze()
