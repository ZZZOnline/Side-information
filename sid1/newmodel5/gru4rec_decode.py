...
class gru4rec_decode(torch.nn.Module):
    ...
    def __init__(self, config, dataset, args):
        super(gru4rec_decode, self).__init__()
    ...
    def forward(self, item_seq, item_seq_len, beha_seq):
        ...
        beha_emb = None
        hidden_states = self.decoder(gru_output, item_emb, beha_emb, extended_attention_mask)
        itemoutput = hidden_states + gru_output
        ...
        fea_output = []
        for i, name in enumerate(self.selected_features):
            fea_hidden = self.decoder1(gru_output, feature_table[i].squeeze(), beha_emb, extended_attention_mask)
