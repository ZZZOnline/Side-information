...
#with beha embedding
class ours4_sas1(torch.nn.Module):
    r"""
    ...
    def __init__(self, config, dataset, args):
        super(ours4_sas1, self).__init__()
    ...
    def decoder(self, input, value_input, beha_emb, extended_attention_mask):
        ...
        mixed_query_layer = self.query(input+pos_emb)
        mixed_key_layer = self.key(input+pos_emb)
    ...
    def decoder1(self, input, value_input, beha_emb, extended_attention_mask):
        ...
        # beha_emb = self.dropout(beha_emb)
        value_input = self.dropout(value_input)
        mixed_query_layer = self.query1(input+pos_emb)
        mixed_key_layer = self.key1(input+pos_emb)
