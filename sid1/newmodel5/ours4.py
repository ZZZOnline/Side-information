...
class ours4(torch.nn.Module):
    ...
    def __init__(self, config, dataset, args):
        ...
        self.gating_network1 = VanillaAttention1(self.hidden_size, self.hidden_size)
    ...
    def decoder(self, input, value_input, beha_emb, extended_attention_mask):
        ...
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))+torch.matmul(query_layer0, (pos_key_layer).transpose(-1, -2)/2)+\
                    torch.matmul(( pos_query_layer)/2, key_layer0.transpose(-1, -2))
