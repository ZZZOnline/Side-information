...
class ours6(torch.nn.Module):
    ...
    def forward(self, item_seq, item_seq_len, beha_seq):
        ...
        boost = torch.softmax(torch.cat((self.boost1, boost), dim=-2), dim=-2)
        # print(boost.shape)
        boost_list = torch.split(boost, 1, dim=-2)
        # print(len(boost_list), boost_list)
        ...
        itemoutput = torch.einsum('bnhdl, hlk->bnhdk', itemoutput, boost_list[0])
        ...
        for i, name in enumerate(self.selected_features):
            ...
            fea_hidden = torch.einsum('bnhdl, hlk->bnhdk', fea_hidden, boost_list[i+1])
    ...
    def calculate_loss(self, interaction):
        ...
        for i, name in enumerate(self.selected_features):
            ...
            result = torch.einsum('bnhdl, hlk->bnhdk', result, boost_list[i+1])
        ...
        if args.integra =='add':
            ...
            catetest_item_emb = torch.einsum('bnhdl, hlk->bnhdk', catetest_item_emb, boost_list[0])
