...
class sasrec_catey(torch.nn.Module):
    r"""
    ...
    def __init__(self, config, dataset, args):
        super(sasrec_catey, self).__init__()
        ...
        for _ in range(self.n_layers):
            ...
            new_attn_layer =  novaMultiHeadAttention1(self.n_heads, self.hidden_size, self.hidden_dropout_prob, self.attn_dropout_prob, self.layer_norm_eps)
    ...
    def forward(self, item_seq, new_item, item_seq_len):
        ...
        for i, name in enumerate(self.selected_features):
            if self.dataset.field2type[name] == FeatureType.TOKEN:
                ...
                feature_table1.append(self.feat_emb_layers[i](self.feat_all[i][new_item].to(item_seq.device)).unsqueeze(-2))
                ...
                if self.args.pooling_mode=='sum':
                    ...
                    feat_emb1 = torch.sum(self.feat_emb_layers[i](self.feat_all[i][new_item].to(item_seq.device)), dim=-2).unsqueeze(-2)
                    # feat_emb = self.transpose_for_scores(feat_emb)
                    feature_table.append(feat_emb)
                    feature_table1.append(feat_emb1)
                    ...
                    token_seq_field = self.feat_all[i][new_item].to(item_seq.device)
                    ...
                    result = torch.div(result, value_cnt + eps)  # [batch_size, max_item_length, embed_dim]
                    result = result.unsqueeze(-2)  # [batch_size, max_item_length, 1, embed_dim]
                    feature_table1.append(result)

    
        cate_emb = feature_table[0].squeeze()
        catey_emb = feature_table1[0].squeeze()
        input_emb = self.dropout(cate_emb)
        input_emb1 = self.dropout(catey_emb)
        ...
        for i in range(1):
            ...
            item_input = self.attention_layers[i](input_emb1, input_emb, extended_attention_mask, item_input)
    ...
    def calculate_loss(self, interaction):
        ...
        new_id_list = torch.cat((item_seq[:,1:], torch.zeros((item_seq.shape[0],1), dtype=torch.int64, device=pos_items.device)), dim=-1)
        new_id_list[torch.arange(0,item_seq.shape[0]), item_seq_len-1] = pos_items

        seq_output = self.forward(item_seq, new_id_list, item_seq_len)
