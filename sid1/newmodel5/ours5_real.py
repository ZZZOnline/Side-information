...
#5 add additional adaptive weight in the embedding part
class ours5_real(torch.nn.Module):
    r"""
    ...
    def __init__(self, config, dataset, args):
        super(ours5_real, self).__init__()
        ...
        self.feat_boost1 = nn.ParameterList()
        for i, field_name in enumerate(config['selected_features']):
            ...
            self.feat_boost1.append(torch.nn.Parameter(torch.ones((self.p_heads, 1, 1), requires_grad=True)))
    ...
    def calculate_loss(self, interaction):
        ...
        for i, name in enumerate(self.selected_features):
            ...
            result = torch.einsum('bnhdl, hlk->bnhdk', result, self.feat_boost1[i])
        ...
        if args.integra =='add':
            ...
            catetest_item_emb = torch.einsum('bnhdl, hlk->bnhdk', catetest_item_emb, self.boost2)
            ...
            if torch.rand(1)<0.01:
                print(self.boost1, self.feat_boost[0], self.boost2, self.feat_boost1[0])
            
            boostloss = self.boost1.norm()+self.boost2.norm()
            for i in self.feat_boost:
                boostloss += i.norm()
            for i in self.feat_boost1:
                boostloss += i.norm()
