...
#real1:先动态混合decoder输出，再残差连接backbone输出
class ours3_real1(torch.nn.Module):
    r"""
    ...
    def __init__(self, config, dataset, args):
        super(ours3_real1, self).__init__()
    ...
    def forward(self, item_seq, item_seq_len, beha_seq):
        ...
        itemoutput = hidden_states
        origin_itemoutput = self.gather_indexes(itemoutput, item_seq_len - 1)
        back_output = self.gather_indexes(output, item_seq_len - 1)
        ...
        # itemoutput squeezed
        return itemoutput.squeeze(), fea_output, origin_itemoutput, back_output
    ...
    def calculate_loss(self, interaction):
        ...
        # itemoutput squeezed
        itemoutput, fea_output, origin_itemoutput, back_output = self.forward(item_seq, item_seq_len, beha_seq)
        args = self.args
