...
class sasrec_catea(torch.nn.Module):
    r"""
    ...
    def __init__(self, config, dataset, args):
        super(sasrec_catea, self).__init__()
    ...
    def forward(self, item_seq, item_seq_len):
        ...
        item_v = self.dropout(item_emb)
        extended_attention_mask = self.get_attention_mask(item_seq)

        for i in range(1):
            item_input = self.attention_layers[i](item_input, extended_attention_mask, item_v)
