...
class difsr_decode(torch.nn.Module):
    """
    ...
    def __init__(self, config, dataset, args):
        super(difsr_decode, self).__init__()
    ...
    def forward(self, item_seq, item_seq_len, beha_seq):
        ...
        for i, name in enumerate(self.selected_features):
            ...
            fea_hidden = self.gather_indexes(fea_hidden, item_seq_len - 1)
        ...
        return self.boost1*itemoutput, fea_output, output
    ...
    def calculate_loss(self, interaction):
        ...
        if True:  # self.loss_type = 'CE'
            ...
            if self.attribute_predictor!='' and self.attribute_predictor!='not':
                ...
                total_loss = loss
            return total_loss + self.weight_norm*(self.boost1.norm()+self.boost2.norm())
