...
#q(i)k(i,j)  +  q_{i,j}, k(j)

class ours4_d1(torch.nn.Module):
    r"""
    ...
    def decoder(self, input, value_input, beha_emb, extended_attention_mask, item_seq_len):
        
        #50
        position_ids = torch.arange(input.size(1), 0, -1, dtype=torch.long, device=input.device)
        position_ids = position_ids.unsqueeze(0)

        #[1,50,d]
        position_embedding = self.position_embedding1(position_ids)

        # self.position_embedding1 = ()
        ...
        #[b,1,d]
        mixed_query_layer = self.gather_indexes(mixed_query_layer, item_seq_len-1).unsqueeze(1)
        ...
        #[b,h,,1,d] [b,h,n,d] [b,h,1,n]
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))
        
        #c2p  [b,h,,1,d] [b,h,n,d] [b,h,1,n]
        c2p_att = torch.matmul(query_layer, pos_key_layer.transpose(-1, -2))


        attention_scores +=c2p_att


        #p2c [n,d] []
        p2c_att = torch.einsum('chnd, bhnd->bchn',pos_query_layer, key_layer).squeeze()

        #torch.Size([256, 4, 1, 50]) torch.Size([256, 1, 4, 50]) torch.Size([1, 4, 50, 32])
        # print(attention_scores.shape, p2c_att.shape, pos_query_layer.shape)
        # p2c_att = torch.gather(p2c_att, dim=-2, index=c2p_pos)

        attention_scores += p2c_att.unsqueeze(-2)
        ...
        hidden_states = self.out_dropout(hidden_states).squeeze()

        return self.LayerNorm(hidden_states)

    def decoder1(self, input, value_input, beha_emb, extended_attention_mask, item_seq_len):
        #50
        position_ids = torch.arange(input.size(1), 0, -1, dtype=torch.long, device=input.device)
        position_ids = position_ids.unsqueeze(0)
    ...
    def forward(self, item_seq, item_seq_len, beha_seq):
        ...
        #[b,1,1,d]
        mask = self.gather_indexes(extended_attention_mask.squeeze(), item_seq_len - 1).unsqueeze(1).unsqueeze(2)
        ...
        beha_emb = None
        hidden_states = self.decoder(output, item_emb, beha_emb, mask, item_seq_len)
        # print('hidden_states', hidden_states.shape)
        itemoutput = hidden_states + self.gather_indexes(output, item_seq_len - 1)
        origin_itemoutput = itemoutput

        # itemoutput = self.gather_indexes(itemoutput, item_seq_len - 1).unsqueeze(1)
        itemoutput = self.transpose_for_heads(itemoutput.unsqueeze(1), args.p_heads)
        ...
        fea_output = []
        for i, name in enumerate(self.selected_features):
            fea_hidden = self.decoder1(output, feature_table[i].squeeze(), beha_emb, mask, item_seq_len)
            # print('fea_hidden',fea_hidden.shape )
            # fea_hidden = self.gather_indexes(fea_hidden, item_seq_len - 1).unsqueeze(1)
            fea_hidden = self.transpose_for_heads(fea_hidden.unsqueeze(1), args.p_heads)
